...
describe('useTripData', () => {
  ...
  describe('loadTripData', () => {
    ...
    it('should keep the raw collaborator as user id when the object carries none', async () => {
      const rawCollaborator = { role: 'viewer' };
      mockApi.getTripById.mockResolvedValue(makeRawTrip({ collaborators: [rawCollaborator] }));

      const { result } = await renderLoaded();

      expect(result.current.trip?.collaborators[0].userId).toEqual(rawCollaborator);
    });
  ...
  describe('handleUpdateBooking', () => {
    it('should replace only the updated booking in the list', async () => {
      mockApi.getBookingsByTripId.mockResolvedValue([
        makeRawBooking(),
        makeRawBooking({ _id: 'book2', title: 'Train de nuit' }),
      ]);
      ...
      expect(result.current.bookings[1].title).toBe('Train de nuit');
    });
  ...
  describe('handleValidateTrip', () => {
    it('should ask for confirmation before validating the trip', async () => {
      ...
      await act(async () => {
        await result.current.handleValidateTrip();
      });
